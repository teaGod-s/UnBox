...
> ⚠️ **已过时提醒**：§3.4 的 `mpvlib`（libmpv + CAMetalLayer）macOS 渲染方案已在
> **M4 废弃**，三平台统一为「内置 Web 播放 + 外部 mpv」；详见
> `2026-08-25-unbox-m4-playback-design.md`。本文件其余部分（配置解析、M3U 导入、
> 直播浏览/播放、测速/故障切换、收藏）仍有效。
